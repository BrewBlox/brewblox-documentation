...
const activeLength = ref('cm');
const activeVoltage = ref('230V');

function toggleLength() {
  activeLength.value = activeLength.value === 'cm' ? 'inch' : 'cm';
}

function toggleVoltage() {
  activeVoltage.value = activeVoltage.value === '230V' ? '240V' : '230V';
}
...
const tableValues = computed(() =>
  props.columns
    ...
    .filter(
      (c) =>
        (!c.voltage && !c.length) ||
        c.voltage === activeVoltage.value ||
        c.length === activeLength.value,
    ),
);

const numRows = computed(() =>
  Math.max(...props.columns.map((c) => c.values.length)),
);
</script>
...
<template>
  <div>
    <div class="table-container">
      <div class="button-prefix">Length unit:</div>
      <div
        class="table-button"
        @click="toggleLength"
      >
      ...
      <div class="button-prefix">Voltage:</div>
      <div
        class="table-button"
        @click="toggleVoltage"
      >
